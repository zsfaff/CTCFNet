...
def rand_flip_MCD(img1, label1, label_mask):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, label1, label_mask
    elif r < 0.5:
        return np.flip(img1, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label_mask, axis=0)
    elif r < 0.75:
        return np.flip(img1, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label_mask, axis=1)
    else:
        return img1[::-1, ::-1, :].copy(),  label1[::-1, ::-1].copy(), label_mask[::-1, ::-1].copy(),
...
def rand_rot90_MCD(img1, label1, label_mask):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, label1, label_mask
    else:
        return np.rot90(img1).copy(), np.rot90(label1).copy(), np.rot90(label_mask).copy()
...
def rand_rot90_flip_MCD(img1, label1, label_mask):
    img1, label1, label_mask = rand_rot90_MCD(img1, label1, label_mask)
    return rand_flip_MCD(img1, label1, label_mask)
